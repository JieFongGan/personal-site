...
export function initHero() {
  ...
  // 1. SETUP CLIENT-SIDE CONTENT (Icons & Greetings)
  const sunIcon = document.getElementById("hero-sun");
  const moonIcon = document.getElementById("hero-moon");
  ...
  if (greetingContainer) {
    ...
    greetings.forEach((text) => {
      ...
      greetingContainer.appendChild(outerSpan);
    });
  }

  // 2. INITIALIZE GSAP ENTRANCE
  // Override CSS hidden state
...
function initClockLogic() {
  ...
  function updateClock() {
    ...
    // Grabs the timezone abbreviation (e.g., GMT, PST)
    const newTz = now.toLocaleTimeString("en-US", { timeZoneName: "short" }).split(" ").pop() || "LOCAL";
    
    if (clock.textContent !== newTime) clock.textContent = newTime;
    if (tz.textContent !== newTz) tz.textContent = newTz;
  }
  updateClock();
...
function initBackgroundAnimations() {
  // Float only the visible SVG
  const activeIcon = document.querySelector(".hero-icon svg:not(.hidden)");
  if (activeIcon) {
    gsap.to(activeIcon, { 
      y: -20, 
      duration: 3, 
